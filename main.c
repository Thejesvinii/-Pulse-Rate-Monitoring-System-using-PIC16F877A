    ...
    if (N > 250) { // avoid high frequency noise
        if ((Signal > thresh) && (Pulse == false) && (N > (IBI / 5)*3)) {
            ...
            for (i = 0; i <= 8; i++) { // shift data in the rate array
                ...
                runningTotal += (unsigned int)(rate[i]); // add up the 9 oldest IBI values
            }

            rate[9] = IBI; // add the latest IBI to the rate array
            runningTotal += (unsigned int)(rate[9]); // add the latest IBI to runningTotal
            ...
            case SHOW_HEART_BEAT:
            {
                if (QS == true) { // A Heartbeat Was Found
                   ...
                   // tune = BPM / 2;
                    ...
                    lcd_com(0x80);                  
                    lcd_puts("BPM:-     ");
                    lcd_print_number((unsigned int)(BPM));
                    lcd_puts (" ");
                    lcd_com(0xC0);
                    lcd_puts("I.B.I:-   ");
                    lcd_print_number((unsigned int)(IBI));
                    lcd_puts (" ");
